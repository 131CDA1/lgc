...
if __name__ == '__main__':
    ...
    x, y, y_mixed, labels, label, k = load_data_bypersent(data_raw, true_labels, random_seed=2023, percent=0.02)
    ...
    classes, label_distributions = fit(x, y_mixed, sigma, delta, centers, type='dng', alpha=0.2)
    score, acc = score(classes, y, label_distributions, data_raw, sigma, delta, label, type='dng')
    print(score)
    print('准确度为', acc)
    draw_decision(data_raw, rhos, delta)
    label_drawing(labels, centers, classes, label_distributions, x, sigma, delta, label, type='dng')
